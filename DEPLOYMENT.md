...
Commands below are **PowerShell** syntax (this project's shell). Note: PowerShell's
line-continuation character is a backtick `` ` ``, not `\` — bash-style `\`
continuations get treated as literal characters and silently corrupt the next
argument (this is what caused the `\secretmanager.googleapis.com` error). Each
command here is kept on one line to sidestep the issue entirely.

```powershell
$env:PROJECT_ID = "weddingstudio-498219"
$env:REGION = "europe-west1"          # must match REGION in deploy-cloud-run.yml
...
gcloud artifacts repositories create lumina --repository-format=docker --location=$env:REGION

# Server runtime secrets (the values from your .env)
"YOUR_GEMINI_API_KEY"   | gcloud secrets create GEMINI_API_KEY --data-file=-
"YOUR_GOOGLE_CLIENT_ID" | gcloud secrets create GOOGLE_CLIENT_ID --data-file=-
"YOUR_GOOGLE_SECRET"    | gcloud secrets create GOOGLE_CLIENT_SECRET --data-file=-
```

If `services enable` still fails with a permission/not-found error after fixing
the syntax, it usually means one of:
- The project doesn't have **billing enabled** (Cloud Run/Artifact Registry/Secret
  Manager all require it) — check **GCP Console → Billing**.
- You're not actually targeting the project you think — run `gcloud config get-value project`
  and confirm it matches `weddingstudio-498219` (or whichever project you intend).
- Your account lacks the `roles/serviceusage.serviceUsageAdmin` (or `Editor`/`Owner`)
  role on that project — check **IAM & Admin** in the console.
...
$env:SA = "github-deployer@$($env:PROJECT_ID).iam.gserviceaccount.com"

gcloud projects add-iam-policy-binding $env:PROJECT_ID --member="serviceAccount:$($env:SA)" --role="roles/run.admin"
gcloud projects add-iam-policy-binding $env:PROJECT_ID --member="serviceAccount:$($env:SA)" --role="roles/artifactregistry.writer"
gcloud projects add-iam-policy-binding $env:PROJECT_ID --member="serviceAccount:$($env:SA)" --role="roles/iam.serviceAccountUser"
gcloud projects add-iam-policy-binding $env:PROJECT_ID --member="serviceAccount:$($env:SA)" --role="roles/secretmanager.secretAccessor"

# Workload Identity pool + GitHub provider
gcloud iam workload-identity-pools create github --location=global
gcloud iam workload-identity-pools providers create-oidc github-provider --location=global --workload-identity-pool=github --issuer-uri="https://token.actions.githubusercontent.com" --attribute-mapping="google.subject=assertion.sub,attribute.repository=assertion.repository" --attribute-condition="assertion.repository=='YOUR_GITHUB_USER/lumina-wedding-studio'"

# Let the GitHub repo impersonate the deployer SA
$env:POOL_ID = gcloud iam workload-identity-pools describe github --location=global --format='value(name)'
gcloud iam service-accounts add-iam-policy-binding $env:SA --role="roles/iam.workloadIdentityUser" --member="principalSet://iam.googleapis.com/$($env:POOL_ID)/attribute.repository/YOUR_GITHUB_USER/lumina-wedding-studio"
...
```powershell
keytool -genkeypair -v -keystore release.keystore -alias lumina -keyalg RSA -keysize 2048 -validity 10000
```
